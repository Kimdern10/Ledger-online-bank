...
<div class="send-wrap">
  ...
  <div class="send-card fade-in d2" style="font-size:13.5px; line-height:1.7; color:var(--text-2);">
    <p style="margin:0 0 14px; font-size:12px; color:var(--text-3);">{{ __('terms.last_updated', ['date' => now()->translatedFormat('F Y')]) }}</p>

    @for ($i = 1; $i <= 17; $i++)
      <p style="margin:0 0 14px;"><strong>{{ __("terms.s{$i}_title") }}</strong> {{ __("terms.s{$i}_body") }}</p>
    @endfor

    <p style="margin:0;"><strong>{{ __('terms.s18_title') }}</strong> {!! __('terms.s18_body', ['support_link' => '<a href="' . route('support') . '" style="color:#C9A24B; font-weight:600;">' . __('terms.support_link_text') . '</a>']) !!}</p>
  </div>
