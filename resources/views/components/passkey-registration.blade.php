...
>
    <template x-if="!supported">
        <flux:text><?= e(__('Passkeys are not supported in this browser.')) ?></flux:text>
    </template>
    ...
    <template x-if="supported && showForm">
        <div class="space-y-4 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50 p-4">
            <flux:input
                label="<?= e(__('Passkey name')) ?>"
                x-model="name"
                placeholder="<?= e(__('e.g., MacBook Pro, iPhone')) ?>"
                x-on:keydown.enter.prevent="register()"
                x-ref="passkeyNameInput"
                x-init="$nextTick(() => $refs.passkeyNameInput?.focus())"
            />
            <flux:text class="!mt-1"><?= e(__('Give this passkey a name to help you identify it later.')) ?></flux:text>
            ...
            <div class="flex gap-2">
                ...
                >
                    <span x-show="!loading"><?= e(__('Register passkey')) ?></span>
                    <span x-show="loading" x-cloak><?= e(__('Registering...')) ?></span>
                </flux:button>
                <flux:button
                    variant="ghost"
                    x-on:click="cancel()"
