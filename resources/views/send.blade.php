...
<script>
  window.LedgerSendConfig = {
    i18n: @json($sendI18n),
    withdrawStoreUrl: "{{ route('withdraw.store') }}",
    sendStoreUrl: "{{ route('send.store') }}",
    internationalConvertUrl: "{{ route('send.international.convert') }}",
    sendLookupUrl: "{{ route('send.lookup') }}",
    banksSearchUrl: "{{ route('banks.search') }}",
    banksSearchInternationalUrl: "{{ route('banks.search.international') }}",
  };
</script>
@endsection
