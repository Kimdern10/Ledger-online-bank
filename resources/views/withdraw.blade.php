...
<div class="send-wrap">
  ...
  <?php if (session('pinRequired')): ?>
    <div class="send-alert fade-in d1">
      <?= e(__('withdraw.pin_required_prefix')) ?>
      <a href="<?= e(route('setting.transaction-pin')) ?>" style="text-decoration:underline; color:inherit;"><?= e(__('withdraw.pin_required_link')) ?></a>.
    </div>
  <?php endif; ?>
  <?php if ($errors->any()): ?>
    <div class="send-alert fade-in d1">
  ...
  <p class="section-label"><?= e(__('withdraw.from')) ?></p>
  <div class="from-row fade-in d2">
    ...
    <div class="bank-balance">
      ...
      <p class="tag"><?= e(__('withdraw.available')) ?></p>
    </div>
  </div>

  <form method="POST" action="<?= e(route('withdraw.store')) ?>" id="withdrawForm">
    <?= csrf_field() ?>
    ...
    <p class="section-label"><?= e(__('withdraw.withdraw_to')) ?></p>
    <div class="fade-in d2">
      ...
      <div id="withdrawBankPanel">
        <?php if ($bankAccounts->isEmpty()): ?>
          <p style="padding:10px 4px; opacity:0.65; font-size:13px;">
            <?= e(__('withdraw.no_linked_bank')) ?> <a href="<?= e(route('link-account')) ?>" style="color:inherit; text-decoration:underline;"><?= e(__('withdraw.link_one_first')) ?></a>.
          </p>
        <?php else: ?>
          <div class="linked-chips">
            <?php foreach ($bankAccounts as $index => $account): ?>
              <div class="linked-chip <?= e($index === 0 ? 'selected' : '') ?>" data-account-id="<?= e($account->id) ?>" onclick="selectDestChip(this, 'bank', <?= e($account->id) ?>)">
      ...
      <div id="withdrawCardPanel" style="display:none;">
        <?php if ($cardAccounts->isEmpty()): ?>
          <p style="padding:10px 4px; opacity:0.65; font-size:13px;">
            <?= e(__('withdraw.no_linked_card')) ?> <a href="<?= e(route('link-account')) ?>" style="color:inherit; text-decoration:underline;"><?= e(__('withdraw.link_one_first')) ?></a>.
          </p>
        <?php else: ?>
          <div class="linked-chips">
            <?php foreach ($cardAccounts as $index => $account): ?>
              <div class="linked-chip <?= e($index === 0 ? 'selected' : '') ?>" data-account-id="<?= e($account->id) ?>" onclick="selectDestChip(this, 'card', <?= e($account->id) ?>)">
                <span class="chip-mark" style="background:var(--ink);"><?= e(substr($account->displayLabel(), 0, 1)) ?></span> <?= e($account->displayLabel()) ?> <?= e($account->detailLabel()) ?>
              </div>
            <?php endforeach; ?>
          </div>
          <p class="receive-note">
            <svg viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
            <?= e(__('withdraw.card_note')) ?>
          </p>
        <?php endif; ?>
      </div>
    </div>

    <p class="section-label"><?= e(__('withdraw.amount')) ?></p>
    <div class="amount-card fade-in d3">
      <div class="amount-field">
        <span class="cur">$</span>
        <input type="text" id="amountInput" inputmode="decimal" value="200" aria-label="<?= e(__('withdraw.amount_aria_label')) ?>">
      </div>
      ...
      <div class="fee-breakdown" id="feeBreakdown">
        <div class="fee-row">
          <span><?= e(__('withdraw.fee_amount')) ?></span>
          <span id="feeAmountDisplay">$200.00</span>
        </div>
        <div class="fee-row">
          <span><?= e(__('withdraw.fee_instant_withdrawal')) ?></span>
          <span id="feeFeeDisplay">$1.50</span>
        </div>
        <div class="fee-row total">
          <span><?= e(__('withdraw.fee_you_will_receive')) ?></span>
    ...
    <div class="pay-btn-wrap fade-in d3">
      <button type="submit" class="pay-btn" id="withdrawSubmitBtn" <?= e($bankAccounts->isEmpty() && $cardAccounts->isEmpty() ? 'disabled' : '') ?>><?= e(__('withdraw.withdraw_btn')) ?></button>
    </div>
  </form>
...
<div class="processing-overlay" id="processingOverlay">
  ...
  <p class="processing-text"><?= e(__('withdraw.processing')) ?></p>
  <p class="processing-sub"><?= e(__('withdraw.processing_sub')) ?></p>
</div>
...
<div class="sheet-overlay" id="pinModalOverlay" onclick="closePinModal(event)">
  <div class="sheet" onclick="event.stopPropagation()" style="max-width:340px; margin:0 auto;">
    ...
    <div class="sheet-head">
      <h4><?= e(__('withdraw.pin_modal_title')) ?></h4>
    ...
    <div style="padding:4px 20px 22px;">
      <p style="margin:0 0 14px; font-size:13px; color:var(--text-3);"><?= e(__('withdraw.pin_modal_body')) ?></p>
      ...
      <button type="button" class="pay-btn" style="margin-top:16px; width:100%;" onclick="confirmPin()"><?= e(__('withdraw.pin_confirm')) ?></button>
    </div>
  </div>
</div>

<script>
  window.LedgerWithdrawConfig = {
    linkAccountFirstAlert: <?= json_encode(__('withdraw.link_account_first_alert')) ?>,
    pinError: <?= json_encode(__('withdraw.pin_error')) ?>,
  };
</script>
@endsection
