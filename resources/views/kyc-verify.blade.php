...
<div class="send-wrap">
  ...
  <div class="page-header fade-in d1">
    ...
    <h1>{{ __('verify.verify_identity') }}</h1>
  ...
  @if($kyc && $kyc->isApproved())
    ...
    <div class="card-detail-card fade-in d2" style="text-align:center; padding:36px 24px;">
      ...
      <p style="margin:0 0 8px; font-size:16.5px; font-weight:700;">{{ __('verify.kyc_approved_title') }}</p>
      <p style="margin:0 auto 22px; max-width:380px; font-size:13.5px; opacity:0.75; line-height:1.55;">
        @if($kyc->reviewed_at)
          {{ __('verify.kyc_approved_body_dated', ['date' => $kyc->reviewed_at->translatedFormat('M j, Y')]) }}
        @else
          {{ __('verify.kyc_approved_body') }}
        @endif
      </p>
      <a href="{{ route('dashboard') }}" class="btn btn-primary">{{ __('verify.back_to_dashboard') }}</a>
    </div>
  ...
  @elseif($kyc && $kyc->isPending())
    ...
    <div class="card-detail-card fade-in d2" style="text-align:center; padding:36px 24px;">
      ...
      <p style="margin:0 0 8px; font-size:16.5px; font-weight:700;">{{ __('verify.kyc_pending_title') }}</p>
      <p style="margin:0 auto 22px; max-width:380px; font-size:13.5px; opacity:0.75; line-height:1.55;">
        {{ __('verify.kyc_pending_body', ['type' => $kyc->documentTypeLabel(), 'time' => $kyc->created_at->diffForHumans()]) }}
      </p>
      <a href="{{ route('dashboard') }}" class="btn btn-primary">{{ __('verify.back_to_dashboard') }}</a>
    </div>
    ...
    @if($kyc && $kyc->isRejected())
      <div class="send-alert fade-in d2">
        ...
        @if($kyc->rejection_reason)
          <p style="margin:6px 0 0;">{{ $kyc->rejection_reason }}</p>
        @endif
        <p style="margin:6px 0 0;">{{ __('verify.kyc_resubmit_notice') }}</p>
      </div>
    @endif

    <div class="send-card fade-in d2">
      <p style="margin:0 0 4px; font-size:15px; font-weight:700;">{{ __('verify.kyc_form_title') }}</p>
      <p style="margin:0 0 20px; font-size:13px; color:var(--text-3); line-height:1.5;">
        {{ __('verify.kyc_form_body') }}
      </p>
      ...
      <form method="POST" action="{{ route('kyc.store') }}" enctype="multipart/form-data">
        ...
        <div class="field-group">
          <p class="label">{{ __('verify.id_type_label') }}</p>
          <div class="select-wrap">
            <select name="id_document_type" class="text-input" required>
              <option value="">{{ __('verify.select_id_type') }}</option>
              @foreach($documentTypes as $value => $label)
                <option value="{{ $value }}" {{ old('id_document_type', $kyc->id_document_type ?? '') === $value ? 'selected' : '' }}>{{ $label }}</option>
              @endforeach
            </select>
            <div class="chev">
        ...
        <div class="field-group">
          <p class="label">{{ __('verify.id_photo_label') }}</p>
          <input type="file" name="id_document" class="text-input" accept="image/*" required>
        </div>

        <div class="field-group">
          <p class="label">{{ __('verify.selfie_label') }} <span style="font-weight:400; color:var(--text-3);">{{ __('verify.selfie_hint') }}</span></p>
