...
<div class="send-wrap">
  ...
  <div class="profile-card fade-in d2" style="margin-bottom:18px;">
    ...
    <p class="profile-email">{{ __('appinfo.version', ['number' => '1.0.0']) }}</p>
  </div>

  <div class="fade-in d3">
    <p class="setting-group-label">{{ __('appinfo.details') }}</p>
    <div class="setting-list">
      <div class="setting-item" style="cursor:default;">
        <span class="label">{{ __('appinfo.app_version') }}</span>
        <span style="font-size:13px; color:var(--text-3);">1.0.0</span>
      </div>
      <div class="setting-item" style="cursor:default;">
        <span class="label">{{ __('appinfo.laravel') }}</span>
        <span style="font-size:13px; color:var(--text-3);">{{ app()->version() }}</span>
      </div>
      <div class="setting-item" style="cursor:default;">
        <span class="label">{{ __('appinfo.php') }}</span>
        <span style="font-size:13px; color:var(--text-3);">{{ PHP_VERSION }}</span>
      </div>
    </div>

    <p class="setting-group-label">{{ __('appinfo.more') }}</p>
    <div class="setting-list">
