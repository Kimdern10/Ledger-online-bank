...
<div class="send-wrap">
  ...
  <div id="scanPanel" class="fade-in d3">
    ...
    <p class="scan-status" id="scanStatus"><?= e(__('scan.scan_status_default')) ?></p>
    <canvas id="scanCanvas" style="display:none;"></canvas>

    <div class="scan-fallback">
      <input type="text" class="text-input" id="manualCodeInput" placeholder="<?= e(__('scan.manual_code_placeholder')) ?>">
      <button type="button" class="pay-btn" onclick="submitManualCode()"><?= e(__('scan.go_button')) ?></button>
    </div>
  </div>
  ...
  <div id="myQrPanel" class="fade-in d3" style="display:none;">
    <div class="send-card">
      <p class="ledger-label" style="text-align:center; margin-bottom:14px;"><?= e(__('scan.show_this_to_get_paid')) ?></p>
      <div class="qr-box">
        <div id="qrcode"></div>
      </div>
      <p class="qr-caption"><?= e($user->name) ?> · <?= e($bankName) ?> · <?= e($accountLabel) ?> •••• <?= e(substr((string) $primaryAccountNumber, -4)) ?></p>
    </div>
  </div>
...
<div class="ledger-toast" id="ledgerToast">
  ...
  <span id="toastText"><?= e(__('scan.toast_code_recognized')) ?></span>
</div>
...
<script>
  window.LedgerScanConfig = {
    ...
    sendUrl: "<?= e(route('send')) ?>",
    qrText: 'ledger://pay?name=<?= e(urlencode($user->name)) ?>&account=<?= e($primaryAccountNumber) ?>&bank=<?= e(urlencode($bankName)) ?>',
    i18n: {
      scanStatusDefault: <?= json_encode(__('scan.scan_status_default')) ?>,
      cameraUnavailable: <?= json_encode(__('scan.camera_unavailable')) ?>,
      lookingUpAccount: <?= json_encode(__('scan.looking_up_account')) ?>,
      accountFoundOpeningSend: <?= json_encode(__('scan.account_found_opening_send')) ?>,
      noLedgerAccountFound: <?= json_encode(__('scan.no_ledger_account_found')) ?>,
      couldNotLookUp: <?= json_encode(__('scan.could_not_look_up')) ?>,
    },
  };
</script>
@endsection
