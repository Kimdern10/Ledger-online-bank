...
<div class="send-wrap">
  ...
  <div class="page-header fade-in d1">
    ...
    <h1><?= e(__('setting.monthly_budget')) ?></h1>
  ...
  <div class="send-card fade-in d2">
    <p style="margin:0 0 18px; font-size:13px; color:var(--text-3); line-height:1.5;">
      <?= e(__('account.budget_intro')) ?>
    </p>

    <form method="POST" action="<?= e(route('setting.budget.update')) ?>">
      <?= csrf_field() ?>

      <div class="field-group">
        <p class="label"><?= e(__('account.monthly_budget_field')) ?></p>
        <input type="number" class="text-input" name="monthly_budget" step="0.01" min="1" max="999999.99" value="<?= e(old('monthly_budget', $currentBudget)) ?>" required>
      </div>

      <div class="pay-btn-wrap" style="margin-top:8px;">
        <button type="submit" class="pay-btn"><?= e(__('account.save_budget')) ?></button>
      </div>
    </form>
  </div>
