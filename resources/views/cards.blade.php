...
<div class="send-wrap">
  ...
  <div class="card-scroll fade-in d2">
    ...
    @empty
      <p style="padding:20px 4px; opacity:0.65;">You don't have any cards yet.</p>
    @endforelse
  ...
  @if($latestRequest && $latestRequest->isPending())
    <div class="card-detail-card fade-in d2" style="margin-bottom:18px;">
      <p style="margin:0; font-size:13.5px;">Your request for a {{ strtolower($latestRequest->typeLabel()) }} is awaiting admin approval.</p>
    </div>
  @elseif($latestRequest && $latestRequest->isDeclined())
    <div class="card-detail-card fade-in d2" style="margin-bottom:18px;">
      <p style="margin:0 0 4px; font-size:13.5px; font-weight:600;">Your last card request was declined.</p>
      @if($latestRequest->decline_reason)
        <p style="margin:0; font-size:12.5px; opacity:0.7;">{{ $latestRequest->decline_reason }}</p>
      @endif
      <p style="margin:6px 0 0; font-size:12.5px; opacity:0.7;">Tap "Add a card" above to request another one.</p>
    </div>
  @endif
  ...
  @if($cards->isNotEmpty())
    ...
    <div class="actions fade-in d3" style="padding-top:12px;">
      <button type="button" class="action" onclick="toggleFreeze()">
        ...
        <span id="freezeActionLabel">Freeze</span>
      </button>
    ...
    <div class="card-detail-card fade-in d3">
      ...
      <div class="card-detail-row">
        <div>
          <p class="cd-label">Cardholder</p>
          <p class="cd-value" id="cardholderValue">-</p>
        </div>
      </div>
      <div class="card-detail-row">
        <div>
          <p class="cd-label">Expiry</p>
          <p class="cd-value" id="cardExpiryValue">-</p>
        </div>
        <div>
          <p class="cd-label">CVV</p>
          <p class="cd-value" id="cardCvvValue">•••</p>
        </div>
      </div>
      <div class="card-detail-row">
        <div>
          <p class="cd-label">PIN</p>
          <p class="cd-value" id="cardPinValue">••••</p>
        </div>
        <div class="cd-actions">
          <button type="button" class="icon-mini-btn" onclick="changePin()" title="Change PIN">
      ...
      <div class="card-detail-row">
        <div>
          <p class="cd-label">Spending limit</p>
          <p class="cd-value" id="cardLimitValue">-</p>
        </div>
        <div class="cd-actions">
          <button type="button" class="icon-mini-btn" onclick="editLimit()" title="Change limit">
            <svg viewBox="0 0 24 24" fill="none" stroke-width="1.7"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
          </button>
        </div>
      </div>
    ...
    <div class="card-detail-card fade-in d3">
      <div class="freeze-row">
        <div class="fr-text">
          <p class="t">Freeze this card</p>
          <p class="d">Instantly blocks new purchases and payments</p>
        </div>
    ...
    <!-- ============ card settings ============ -->
    <p class="section-label">Card settings</p>
    <div class="setting-list fade-in d3" style="margin-bottom:18px;">
      <a href="javascript:void(0)" class="setting-item" onclick="editLimit()">
        ...
        <span class="label">Spending limit</span>
      ...
      <a href="javascript:void(0)" class="setting-item" onclick="toggleContactless()">
        ...
        <span class="label">Contactless payments</span>
      ...
      <a href="javascript:void(0)" class="setting-item" onclick="revealPin()">
        ...
        <span class="label">View PIN</span>
        <svg class="s-chev" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6"/></svg>
      </a>
    </div>
    ...
    <div class="setting-list fade-in d3">
      <a href="javascript:void(0)" class="setting-item danger" onclick="reportLostOrStolen()">
        ...
        <span class="label">Report lost or stolen</span>
        <svg class="s-chev" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6"/></svg>
      </a>
      <a href="javascript:void(0)" class="setting-item danger" onclick="closeCard()">
        ...
        <span class="label">Close this card</span>
        <svg class="s-chev" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6"/></svg>
      </a>
    </div>

    <!-- ============ recent card activity ============ -->
    <p class="section-label" style="margin-top:22px;">Recent activity</p>
    <div class="tx-card fade-in d3" id="cardActivityList"></div>
    <p id="cardActivityEmpty" style="display:none; padding:16px 4px; opacity:0.65;">No activity on this card yet.</p>
  @endif
...
<script>
            ...
            'transactions' => $c->transactions->map(function ($t) {
                return [
                    ...
                    'dateLabel' => $t->occurred_at->isToday()
                        ? 'Today'
                        : ($t->occurred_at->isYesterday() ? 'Yesterday' : $t->occurred_at->format('M j')),
                ];
            })->values(),
        ]];
    });
  ...
  function renderCard(){
    ...
    if (contactlessState) contactlessState.textContent = card.contactless ? 'On' : 'Off';

    const onlineState = document.getElementById('onlineState');
    if (onlineState) onlineState.textContent = card.onlinePayments ? 'On' : 'Off';

    document.getElementById('freezeToggle').checked = card.frozen;
    document.getElementById('freezeActionLabel').textContent = card.frozen ? 'Unfreeze' : 'Freeze';
  ...
  function onFreezeToggle(){
    ...
    fetch(urlFor('freeze'), {
      ...
      .then(function(json){
        ...
        showToast(json.status === 'frozen' ? 'Card frozen' : 'Card unfrozen');
      })
      .catch(function(){
        applyFreezeUi(wasFrozen);
        showToast('Could not update card. Try again.');
      });
  }
  ...
  function applyFreezeUi(frozen){
    ...
    badge.textContent = frozen ? 'Frozen' : (CARDS[currentCard].virtual ? 'Virtual' : 'Active');
    document.getElementById('freezeToggle').checked = frozen;
    document.getElementById('freezeActionLabel').textContent = frozen ? 'Unfreeze' : 'Freeze';
  }

  function copyCardNumber(){
    const card = CARDS[currentCard];
    navigator.clipboard?.writeText(card.number.replace(/\s/g,'')).then(()=> showToast('Card number copied'))
  ...
  function revealPin(){
    ...
    showToast('PIN revealed below');
  }

  function changePin(){
    const card = CARDS[currentCard];
    const current = card.hasPin ? prompt('Enter your current 4-digit PIN:') : null;
    if (card.hasPin && current === null) return;
    if (card.hasPin && !/^\d{4}$/.test(current || '')) { showToast('PIN must be 4 digits'); return; }

    const next = prompt('Enter a new 4-digit PIN:');
    if (next === null) return;
    if (!/^\d{4}$/.test(next)) { showToast('PIN must be 4 digits'); return; }

    const confirmPin = prompt('Re-enter the new PIN to confirm:');
    if (confirmPin === null) return;
    if (confirmPin !== next) { showToast('PINs did not match'); return; }
    ...
    fetch(urlFor('pin'), {
      ...
      .then(function(result){
        if (!result.ok) { showToast(result.json.error || 'Could not update PIN'); return; }
        card.pin = next;
        card.hasPin = true;
        if (revealed) renderCard();
        showToast('PIN updated');
      })
      .catch(function(){ showToast('Could not update PIN. Try again.'); });
  }

  function editLimit(){
    const card = CARDS[currentCard];
    const currentAmount = card.limitLabel === 'No limit set' ? '' : card.limitLabel.replace(/[^0-9.]/g, '');
    const raw = prompt('Set a spending limit in dollars (leave blank for no limit):', currentAmount);
    if (raw === null) return;

    const trimmed = raw.trim();
    if (trimmed !== '' && isNaN(Number(trimmed))) { showToast('Enter a valid amount'); return; }
    ...
    fetch(urlFor('limit'), {
      ...
      .catch(function(){ showToast('Could not update limit. Try again.'); });
  }
  ...
  function toggleContactless(){
    ...
    fetch(urlFor('contactless'), {
      ...
      .then(function(json){
        ...
        renderCard();
        showToast('Contactless payments turned ' + (json.enabled ? 'on' : 'off'));
      })
  ...
  function toggleOnlinePayments(){
    ...
    fetch(urlFor('online'), {
      ...
      .then(function(json){
        ...
        renderCard();
        showToast('Online payments turned ' + (json.enabled ? 'on' : 'off'));
      })
      .catch(function(){ showToast('Could not update. Try again.'); });
  }

  function reportLostOrStolen(){
    ledgerConfirm('Report this card lost or stolen? It will be frozen immediately.', function(){
      const form = document.getElementById('reportLostForm');
      form.setAttribute('action', urlFor('reportLost'));
      form.submit();
    }, { danger: true, confirmButtonText: 'Report card' });
  }

  function closeCard(){
    ledgerConfirm('Close this card? This cannot be undone.', function(){
      const form = document.getElementById('closeCardForm');
      form.setAttribute('action', urlFor('close'));
      form.submit();
    }, { danger: true, confirmButtonText: 'Close card' });
  }

  function replaceCard(){
    ledgerConfirm('Replace this card? Your current card number will stop working immediately and a new one will be issued.', function(){
  ...
  function requestNewCard(){
    if (hasPendingCardRequest) {
      showToast('You already have a card request awaiting approval.');
      return;
    }

    const choice = prompt('Request a "physical" or "virtual" card?', 'physical');
    if (choice === null) return;

    const type = choice.trim().toLowerCase();
    if (type !== 'physical' && type !== 'virtual') {
      showToast('Type either "physical" or "virtual"');
      return;
    }

    ledgerConfirm('Request a ' + type + ' card? An admin will need to approve it before it shows up here.', function(){
      document.getElementById('cardRequestTypeField').value = type;
      document.getElementById('cardRequestForm').submit();
    }, { confirmButtonText: 'Request card' });
  }
