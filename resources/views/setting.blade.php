  ...
  <div class="profile-card fade-in d2">
    <div class="profile-avatar">
      <?php if (auth()->user()->avatar ?? false): ?>
        <img src="<?= e(auth()->user()->avatar) ?>" alt="<?= e(auth()->user()->name) ?>" class="profile-avatar" style="border:none; margin:0;">
      <?php else: ?>
        <?php
            // Same crash risk as welcome.blade.php's testimonial avatars: a
            // leading/trailing/double space in the user's stored name makes
            // explode(' ', ...) yield an empty-string part, and PHP 8 throws
            // "Uninitialized string offset 0" on ''[0]. filter() drops those
            // empty parts first so this can't crash the Settings page for a
            // real user with a slightly messy name on file.
        ?>
        <?= e(Str::of(auth()->user()->name ?? 'Cody Lee')->explode(' ')->filter()->map(fn($n) => strtoupper($n[0]))->join('')) ?>
      <?php endif; ?>
    </div>
    <div class="profile-name-row">
      <h3><?= e(auth()->user()->name ?? 'Cody Lee') ?></h3>
      <a href="<?= e(route('setting.profile')) ?>" class="profile-edit">
        <svg viewBox="0 0 24 24" fill="none" stroke-width="1.8"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
      </a>
    </div>
    <p class="profile-email"><?= e(auth()->user()->email ?? 'codylee@gmail.com') ?></p>
    <span class="linked-tag" style="margin-top:10px;"><?= e(auth()->user()->tierLabel()) ?> &middot; $<?= e(number_format(auth()->user()->tierDailyLimit())) ?>/day</span>
  </div>

  <div class="fade-in d3">
    <p class="setting-group-label"><?= e(__('setting.account_group')) ?></p>
    <div class="setting-list">
      <a href="<?= e(route('setting.profile')) ?>" class="setting-item">
        <div class="s-icon">
          <svg viewBox="0 0 24 24" fill="none" stroke-width="1.7"><circle cx="12" cy="8" r="4"/><path d="M4 21v-1a8 8 0 0 1 16 0v1"/></svg>
        </div>
        <span class="label"><?= e(__('setting.profile_setting')) ?></span>
        <svg class="s-chev" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6"/></svg>
      </a>
      <a href="<?= e(route('kyc.create')) ?>" class="setting-item">
        <div class="s-icon">
        ...
        <span class="label"><?= e(__('verify.identity_verification')) ?><span class="linked-tag"><?= e(['approved' => __('verify.badge_verified'), 'pending' => __('verify.badge_pending'), 'rejected' => __('verify.badge_needs_resubmit')][auth()->user()->kyc_status] ?? __('verify.badge_not_started')) ?></span></span>
        <svg class="s-chev" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6"/></svg>
      </a>
      <a href="<?= e(route('address-verification.create')) ?>" class="setting-item">
        <div class="s-icon">
          <svg viewBox="0 0 24 24" fill="none" stroke-width="1.7"><path d="M3 9.5 12 3l9 6.5"/><path d="M5 10v10h14V10"/><path d="M9 20v-6h6v6"/></svg>
        </div>
        <span class="label"><?= e(__('verify.address_verification')) ?><span class="linked-tag"><?= e(['approved' => __('verify.badge_verified'), 'pending' => __('verify.badge_pending'), 'rejected' => __('verify.badge_needs_resubmit')][auth()->user()->address_status] ?? __('verify.badge_not_started')) ?></span></span>
        <svg class="s-chev" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6"/></svg>
      </a>
      <a href="<?= e(route('setting.notifications')) ?>" class="setting-item">
        <div class="s-icon">
        ...
        <span class="label"><?= e(__('setting.notifications_setting')) ?></span>
        <svg class="s-chev" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6"/></svg>
      </a>
      <?php /* No standalone Language settings page anymore — the floating
      ...
      <a href="<?= e(route('setting.budget')) ?>" class="setting-item">
        <div class="s-icon">
        ...
        <span class="label"><?= e(__('setting.monthly_budget')) ?><span class="linked-tag">$<?= e(number_format(auth()->user()->monthlyBudget())) ?></span></span>
        <svg class="s-chev" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6"/></svg>
      </a>
      <?php /* Link Account used to be listed here too, alongside its entry in
      ...
      <?php /* "Banks" (the read-only view of the admin-managed Bank directory,
      ...
      <a href="<?= e(route('setting.transaction-pin')) ?>" class="setting-item">
        <div class="s-icon">
        ...
        <span class="label"><?= e(__('setting.transaction_pin')) ?><span class="linked-tag"><?= e(auth()->user()->hasTransactionPin() ? __('setting.pin_set') : __('setting.pin_not_set')) ?></span></span>
        <svg class="s-chev" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6"/></svg>
      </a>
      <a href="<?= e(route('setting.password')) ?>" class="setting-item">
        <div class="s-icon">
        ...
        <span class="label"><?= e(__('setting.password_change')) ?></span>
        <svg class="s-chev" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6"/></svg>
      </a>
    </div>

    <p class="setting-group-label"><?= e(__('setting.support_group')) ?></p>
    <div class="setting-list">
      ...
      <a href="<?= e(route('setting.app-info')) ?>" class="setting-item">
        <div class="s-icon">
          <svg viewBox="0 0 24 24" fill="none" stroke-width="1.7"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01"/></svg>
        </div>
        <span class="label"><?= e(__('setting.app_info')) ?></span>
        <svg class="s-chev" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6"/></svg>
      </a>
    </div>

    <p class="setting-group-label"><?= e(__('setting.danger_zone')) ?></p>
    <div class="setting-list">
      <a href="<?= e(route('setting.delete-account')) ?>" class="setting-item danger">
        <div class="s-icon">
        ...
        <span class="label"><?= e(__('setting.delete_account')) ?></span>
        <svg class="s-chev" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6"/></svg>
      </a>
      <form method="POST" action="<?= e(route('logout')) ?>" style="display:contents;">
        <?= csrf_field() ?>
        <button type="submit" class="setting-item danger" style="width:100%; border:none; background:none; cursor:pointer; font-family:'Inter', sans-serif; text-align:left;">
          ...
          <span class="label"><?= e(__('setting.logout')) ?></span>
        </button>
      </form>
    </div>
  </div>
